...
double spddd = 60; //secretory progenitor probability (division daughter determinator)
int snapshot = 500;
...
void intraODEs(vector<vector<intra>>& v, vector<vector<extra>>w)
{
    ...
    default_random_engine re(static_cast<unsigned>(time(0)));
    ...
    for (int i = 0; i < gridsize; i++)
    {
        for (int j = 0; j < gridsize; j++)
        {
            ...
            if (v[i][j].ID != 0)
            {
                ...
                if (v[i][j].type == "t")
                {   //2.5% chance on average
                    if (5 - ((w[i][j].WNT / maxWNT) * 10) > unif(re))
                    {
...
void division(vector<vector<intra>>& v, vector<vector<extra>>& w, vector<pair<int, int>>& divpos, vector<int>& divid)
{
    ...
    for (int k = 0; k < divpos.size(); k++)
    {
        ...
        if (v[a][b].ID == divid[k])
        {
            ...
            if (v[a][b].type == "ap")
            {
                if (unif(re) < 50 + (sqrt(pow(a - gridsize / 2, 2) + pow(b - gridsize / 2, 2)) / maxdistance) * 30) //fixed 50%-80% based on distance
                {
...
namespace Simulation {
    static void move(int a, int b, intra temp, vector<vector<intra>>& v)
    {
        if (temp.ID != 0)
        {
            if (a % 2 == 0 && a < gridsize - 1 && b < gridsize - 1 && b>0 && a>0 && (v[a + 1][b - 1].type == "" || v[a - 1][b - 1].type == "" || v[a][b - 1].type == "" || v[a][b + 1].type == "" || v[a + 1][b].type == "" || v[a - 1][b].type == ""))
            {
                ...
                if (v[a - 1][b].type == "") frepos.push_back(make_pair(a - 1, b));
                default_random_engine generator(static_cast<unsigned>(std::time(0)));
                uniform_int_distribution<int> distribution(0, frepos.size()-1);
            ...
            else
            {
                ...
                else
                {
                    ...
                    int r2 = distribution1(generator);
                    
                    uniform_int_distribution<int> distribution2(0, 5);
                    int r6 = distribution2(generator);
        ...
        for (int i = 0; i < gridsize; i++)
        {
            for (int j = 0; j < gridsize; j++)
            {
                ...
                if (v[i][j].type == "p" && it == ids.end())
                {
                    ...
                    ids.push_back(v[i][j].ID);
                    if (unif(re) < sqrt(pow(y, 2) + pow(x, 2)) / maxdistance * 15) //higher distance from origin -> higher chance of switching [0%-10%]
                    {
                        if (y != 0 && x != 0) 
                        {
                            y = y / abs(y);
    ...
    static void show(vector<vector<vector<intra>>> full) // void show and showex functions I used for numerical visualization. normally unused
    {
        for (int k = 0; k < full.size(); k++)
        {
            ...
            for (int i = 0; i < full[k].size(); i++)
            {
                ...
                {
                    if (full[k][i][j].type == "s") numofs++;
                    if (full[k][i][j].type == "t") numoft++;
                    if (full[k][i][j].type == "ap") numofap++;
                    if (full[k][i][j].type == "sp") numofsp++;
                    if (full[k][i][j].type == "p") numofp++;
                    if (full[k][i][j].type == "e") numofe++;
                    if (full[k][i][j].type == "g") numofg++;
                }
                cout << endl;
            }
            
            cout << "Number of ISCs: " << numofs << endl << "Number of TAs: " << numoft << endl << 
                "Number of ap: " << numofap << endl << "Number of sp: " << numofsp << endl << "Number of p: " 
                << numofp << endl << "Number of e: " << numofe << endl << "Number of g: " << numofg << endl;
        }
    }
        ...
        for (int k = 0; k < fullex.size(); k++)
        {
            ...
            for (int i = 0; i < fullex[k].size(); i++)
            {
                for (int j = 0; j < fullex[k][i].size(); j++)
                {
                    ...
                    // cout << full[k][i][j].ID<< " ";
                    PWNT += fullex[k][i][j].pWNT;

                }
                cout << endl;
            }
            cout <<" Sum of Paneth cell produced WNT: "<< endl;
        }
    }
}
...
BOOL MyApp::InitInstance()
{
    ...
    for (size_t i = 0; i < m_data.size(); ++i)
    {
        ...
        if (!pWnd->CreateEx(0, className,
            ...
            560, 560,                          // Window size for gridsize=60
            NULL, NULL))
        {
    ...
    for (int row = 0; row < rows; ++row)
    {
        for (int col = 0; col < cols; ++col)
        {
            ...
            if (type == "s")
                ...
                color = RGB(128, 128, 128);     // Grey for "e"
            else if (type == "t")
...
int main()
{
    ...
    for (int currun = 0; currun <= numofit; currun++)
    {
        ...
        if (currun% snapshot==0)fullex.push_back(exgrid);
        if (currun % snapshot == 0) full.push_back(ingrid); 

    }
    cout << "Number of all cells: " << id << endl;
    Simulation::showex(fullex);
    Simulation::show(full);
    MyApp theApp(full);
    theApp.InitInstance();
    return theApp.Run();
